...
sim_top_dir <- "/home/mikola/slurm_simulator3/sim/micro/scale"
...
```{r}
r<-list()
for(slurm_opt in c("Real","Simulated")){
    r[[slurm_opt]]<-list()
    r[[slurm_opt]]$start_time <- min(sacctRB$Submit[sacctRB$Slurm==slurm_opt])
    r[[slurm_opt]]$end_time <- max(sacctRB$End[sacctRB$Slurm==slurm_opt])
    r[[slurm_opt]]$days<-(as.numeric(r[[slurm_opt]]$end_time)-as.numeric(r[[slurm_opt]]$start_time))/(3600*24)
    
    print(paste(slurm_opt,"ran from",r[[slurm_opt]]$start_time,"to",r[[slurm_opt]]$end_time," or ",r[[slurm_opt]]$days,"days."))
}
#print(paste("Difference:",r$`bf_max_job_user=10`$days-r$`bf_max_job_user=20`$days,"days or",100.0*(r$`bf_max_job_user=10`$days-r$`bf_max_job_user=20`$days)/r$`bf_max_job_user=20`$days,"%"))
